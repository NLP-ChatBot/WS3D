...
public class MindView extends javax.swing.JFrame {
  ...
  public void tick() {
    String alltext = "";
    if (mol.size() != 0) for (MemoryObject mo : mol) {
      if (mo.getI() != null) {
        Object k = mo.getI();
        String moName = mo.getName();
        if (moName.equals("KNOWN_APPLES") || moName.equals("VISION")) {
          alltext += mo.getName() + ": [ ";
          CopyOnWriteArrayList<Thing> l = new CopyOnWriteArrayList<>(
            (List<Thing>) k
          );
          for (Thing t : l) {
          ...
          alltext +=
            ...
            2 +
            ")\n";
        } else alltext += mo.getName() + ": " + k + "\n";
      } else alltext += mo.getName() + ":\n";
    }
    ...
    if (j == 21) {
      try {
        ...
        World.createJewel(2, jewelX - 100, jewelY - 100);
      } catch (Exception e) {
        e.printStackTrace();
      }
      j = 0;
  ...
  public static void main(String args[]) {
    ...
    try {
      ...
      java
        .util.logging.Logger.getLogger(MindView.class.getName())
